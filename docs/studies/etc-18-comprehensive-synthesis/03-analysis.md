...
The etc-master-evidence.md file contains 597 unique evidence items across 17 studies. Each item was registered once when first discovered; subsequent studies reference the existing master ID.

| Tier | Conditionalist | ECT / ECT-direction | Neutral | Total |
|------|----------------|---------------------|---------|-------|
| **Explicit (E)** | 103 | 0 | 346 | 449 |
| **Necessary Implication (N)** | 13 | 0 | 52 | 65 |
...
*Note: One I-B item is classified Neutral (etc-01). ECT-direction includes items marked "ECT" or "ECT-direction" in the master evidence file. One apparent discrepancy (598 vs 597) is due to rounding in the I-B Neutral count; the actual total is 597 deduplicated items.*

**Summary of deduplicated tally:**
- **Conditionalist-direction:** 140 items (23.4% of all items)
  - 103 E-level, 13 N-level, 24 I-A level
  - All conditionalist items are at E, N, or I-A tier (textual evidence or evidence-extending inferences)
- **ECT-direction:** 57 items (9.5% of all items)
  - 0 E-level, 0 N-level, 23 I-B, 30 I-C, 4 I-D
  - ALL ECT items are at the inference level (I-B, I-C, or I-D)
  - No E-item or N-item in the entire series directly supports ECT
- **Neutral:** 401 items (67.1% of all items)
  - 346 E-level, 52 N-level, 2 I-C, 1 I-B
  - The vast majority of evidence is positionally neutral (statements both positions accept)

### 1.2 Raw (Non-Deduplicated) Counts

Raw counts reflect how many times items appear across all 17 studies, counting each "Also In" cross-reference as an additional appearance.

| Tier | Deduplicated | Raw (with cross-refs) | Cross-ref factor |
|------|-------------|----------------------|-----------------|
| E items | 449 | 677 | 1.51x |
| N items | 65 | 80 | 1.23x |
| I items | 83 | 92 | 1.11x |
| **TOTAL** | **597** | **849** | **1.42x** |

The 1.42x cross-reference factor indicates that the average evidence item was examined in approximately 1.42 studies. E-items have the highest cross-reference rate (1.51x) because foundational verses (Gen 2:7, Ecc 9:5,10, Matt 10:28, etc.) are relevant across multiple study topics.

### 1.3 Items per Source Study

| Study | New E | New N | New I | Total New | Topic |
|-------|-------|-------|-------|-----------|-------|
| etc-01 | 57 | 7 | 8 | 72 | Anthropology |
| etc-02 | 48 | 5 | 8 | 61 | Immortality |
| etc-03 | 39 | 4 | 7 | 50 | Death |
| etc-04 | 31 | 5 | 7 | 43 | State of Dead |
| etc-05 | 35 | 5 | 4 | 44 | Four Hell Words |
| etc-06 | 56 | 4 | 4 | 64 | Destruction Vocabulary |
| etc-07 | 28 | 5 | 3 | 36 | Olam (OT "Forever") |
| etc-08 | 23 | 5 | 3 | 31 | Aionios (NT "Forever") |
| etc-09 | 17 | 4 | 3 | 24 | Rich Man & Lazarus |
| etc-10 | 19 | 4 | 3 | 26 | Souls Under Altar |
| etc-11 | 20 | 5 | 4 | 29 | Smoke Ascending |
...
| etc-17 | 23 | 4 | 3 | 30 | God's Character |

Note: etc-01 was deleted (replaced by etc-01). Two E-items originating from the deleted etc-01 remain in the master file (subsequently incorporated into etc-01's scope).
...
|------|-------|--------------------------|-----------|
| E (Explicit) | 103 | 73.6% | Direct biblical statements |
| N (Necessary) | 13 | 9.3% | Unavoidable implications |
| I-A (Extending) | 24 | 17.1% | Systematizing E/N evidence |
| **Total** | **140** | **100%** | |

**Key observation:** 82.9% of conditionalist evidence is at the E or N tier -- direct textual statements and their unavoidable implications. The remaining 17.1% (I-A) systematizes textual evidence without introducing external frameworks.
...
| I-B (Competing) | 23 | 40.4% | Contested verse readings |
| I-C (External) | 30 | 52.6% | Concepts the text does not contain |
| I-D (Counter) | 4 | 7.0% | Overriding biblical word meanings |
| **Total** | **57** | **100%** | |

**Key observation:** 100% of ECT evidence is at the inference level. No explicit biblical statement and no necessary implication in the entire 17-study series was classified as supporting ECT. The ECT case consists entirely of:
- 23 I-B items where both positions claim the same text (all resolved Strong toward conditionalist in prior studies -- meaning the plain-sense reading favors conditionalism while the ECT reading requires additional interpretive moves)
- 30 I-C items that add theological/philosophical concepts not found in any biblical text (e.g., the "sin against infinite God requires infinite punishment" Anselmian argument)
...
```
CONDITIONALIST EVIDENCE (140 items)
E-tier: ████████████████████████████████████████████████████ 103
N-tier: ███████ 13
I-A:    ████████████ 24

ECT EVIDENCE (57 items)
E-tier: (none)
N-tier: (none)
I-B:    ████████████ 23
I-C:    ███████████████ 30
...
The 17 studies naturally fall into five thematic clusters:

**Cluster A: Foundational Anthropology (etc-01, 02, 03, 04)**
...
**Cluster B: Destination Vocabulary (etc-05, 06)**
...
**Cluster C: "Forever" Language (etc-07, 08)**
...
**Cluster E: Contextual Studies (etc-16, 17)**
...
- Evidence character: Historical documentation (etc-16); didactic E-tier passages on divine character (etc-17)

### 3.2 Cross-Cluster Convergence

The five clusters converge on a single coherent picture from independent angles:
...
The 23 I-B items represent passages where both conditionalist and ECT positions claim the same text. Each was resolved in the study where it appeared. Resolution method: compare the Plain vs. Ambiguous character of each side's reading.
...
All 23 I-B items were resolved "Strong" toward conditionalist reading. The resolution pattern is consistent:
...
- **Basis:** 449 E-items were classified across 17 studies. Not a single E-item was classified ECT. Zero.
...
- **Basis:** 65 N-items were classified. Not a single N-item was classified ECT. Zero.
...
The most striking finding across all 17 studies is the structural asymmetry between the two positions:

### Conditionalist Position
- Built on: E-tier and N-tier evidence (116 of 140 items = 82.9%)
...
### ECT Position
- Built on: Inference only (57 of 57 items = 100%)
...
*Analysis completed for etc-18: Comprehensive Synthesis*
